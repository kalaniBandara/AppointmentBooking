...
function PedicureTab() {
  ...
  const OPIPedicures = [
    {
      title: "with OPI Gel Colours",
    },
    {
      title: "with OPI Nail Polish",
    },
    {
      title: "with Non Gel colour",
    },
    {
      title: "with Non polish",
    },
  ];
  const pedicureLists = [
    {
      title: "Lycon with OPI Gel Colours",
    },
    {
      title: "Lycon with OPI Nail Polish",
    },
    {
      title: "Lycon Pedi with Non Gel colour",
    },
    {
      title: "Lycon Pedi with Non polish",
    },
    {
      title: "Non Pedi with OPI Gel",
    },
    {
      title: "Non Pedi with OPI nail Polish",
    },
    {
      title: "Non Pedi with Non Gel Colour",
    },
    {
      title: "Non Pedi with Non Polish",
    },
  ];
  const pedicurevtLists = [
    {
      title: " with OPI Gel Colours",
    },
    {
  ...
  const LashExtentionsLists = [
    {
      title: "Lash Extention Black colour",
    },
    {
      title: "Lash Extention with colours",
    },
    {
      title: "Refill Black",
    },
    ...
    <>
      <div className="w-full flex flex-col items-center justify-start">
        <div className="w-full flex flex-col gap-2">
          ...
          OPIPedicure
          {OPIPedicures.map((OPIPedicure, index) => (
            <div
              className="w-full flex flex-row items-center justify-between ring-2 rounded-lg ring-gr p-2 hover:bg-white transition-all duration-300 hover:cursor-pointer"
              key={index}
            >
              <div className="w-5/6 flex flex-col items-start gap-3">
                <p>{OPIPedicure.title}</p>
              </div>

              <input
                type="radio"
                name="pedicureService"
                value={OPIPedicure.title}
                className="w-5 h-5"
                onClick={() => {
                  setbuttonShow(true);
                  setSelectedService(pedicureList.title);
          ...
          Pedicure
          {pedicurevtLists.map((pedicurevtList, index) => (
            <div
              className="w-full flex flex-row items-center justify-between ring-2 rounded-lg ring-gr p-2 hover:bg-white transition-all duration-300 hover:cursor-pointer"
              key={index}
            >
              <div className="w-5/6 flex flex-col items-start gap-3">
                <p>{pedicurevtList.title}</p>
              </div>

              <input
                type="radio"
                name="pedicureService"
                value={pedicurevtList.title}
                className="w-5 h-5"
                onClick={() => {
                  setbuttonShow(true);
                  setSelectedService(pedicureList.title);
          ...
          Lash Extentions
          {LashExtentionsLists.map((LashExtentionsList, index) => (
            <div
              className="w-full flex flex-row items-center justify-between ring-2 rounded-lg ring-gr p-2 hover:bg-white transition-all duration-300 hover:cursor-pointer"
              key={index}
            >
              <div className="w-5/6 flex flex-col items-start gap-3">
                <p>{LashExtentionsList.title}</p>
              </div>

              <input
                type="radio"
                name="pedicureService"
                value={LashExtentionsList.title}
                className="w-5 h-5"
                onClick={() => {
                  setbuttonShow(true);
                  setSelectedService(pedicureList.title);
